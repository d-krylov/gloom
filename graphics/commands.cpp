...
void SetStencilTesting(bool b) { b ? glEnable(GL_STENCIL_TEST) : glDisable(GL_STENCIL_TEST); }
...
void SetBlending(bool b, Types::BlendEquation equation,
  ...
  if (b) {
    ...
    glBlendFuncSeparate(
      static_cast<uint16_t>(source_rgb), static_cast<uint16_t>(destination_rgb),
      static_cast<uint16_t>(source_alpha), static_cast<uint16_t>(destination_alpha));
  }
}
