...
// clang-format off
template <typename T>
inline void SetUniformInternal(uint32_t shader, int32_t location, T const &value) {
  ...
  else if constexpr (std::is_same_v<T, Types::Matrix4f>) glProgramUniformMatrix4fv(shader, location, 1, GL_FALSE, Types::Cast(value));
  else {
    GLOOM_UNREACHABLE();
  }
}
